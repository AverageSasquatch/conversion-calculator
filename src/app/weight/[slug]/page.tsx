...
// Generate static params for all weight conversions
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return generateConverterMetadata(conversion, "weight");
}

export default async function WeightConverterPage({ params }: PageProps) {
  const { slug } = await params;
  const conversion = getConversionBySlug(slug);

  if (!conversion || conversion.category !== "weight") {
    notFound();
  }
