...
// Generate static params for all time conversions
export async function generateStaticParams() {
  const conversions = getConversionsByCategory("time");
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return generateConverterMetadata(conversion, "time");
}
...
export default async function TimeConverterPage({ params }: PageProps) {
  ...
  if (!conversion || conversion.category !== "time") {
    notFound();
  }
