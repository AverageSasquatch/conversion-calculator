...
// Generate static params for all length conversions
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return generateConverterMetadata(conversion, "length");
}

export default async function LengthConverterPage({ params }: PageProps) {
  const { slug } = await params;
  const conversion = getConversionBySlug(slug);

  if (!conversion || conversion.category !== "length") {
    notFound();
  }
